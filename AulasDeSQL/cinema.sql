...
SELECT SUM(s.publico), c.nome FROM sessao s 
INNER JOIN sala sa ON s.sala_id = sa.id
INNER JOIN cinema ci ON sa.cinema_id = ci.id
INNER JOIN cidade c ON ci.cidade_id = c.id GROUP BY c.nome;

-- 2. Apuração do público por cinema

SELECT SUM(s.publico), ci.nomeFantasia FROM sessao s 
INNER JOIN sala sa ON s.sala_id = sa.id
INNER JOIN cinema ci ON sa.cinema_id = ci.id
...
SELECT s.id, ci.nomeFantasia, SUM(s.publico) FROM sessao s 
INNER JOIN sala sa ON s.sala_id = sa.id
INNER JOIN cinema ci ON sa.cinema_id = ci.id
GROUP BY s.id, ci.nomeFantasia;
...
SELECT DISTINCT ci.nomeFantasia, a.nome, f.tituloOriginal, f.tituloPortugues FROM sessao s INNER JOIN filme f ON s.filme_id = f.id
INNER JOIN sala sa ON s.sala_id = sa.id
INNER JOIN elenco el ON el.filme_id = f.id
INNER JOIN ator a ON el.ator_id = a.id
INNER JOIN cinema ci ON sa.cinema_id = ci.id;

-- 5. Em quais cinemas está sendo exibido um determinado gênero de filme;

SELECT DISTINCT c.nomeFantasia, gf.genero_id, g.nome FROM cinema c
INNER JOIN sala sa ON c.id = sa.cinema_id
INNER JOIN sessao s ON sa.id = s.sala_id
INNER JOIN genero_filme gf ON s.filme_id = gf.filme_id
INNER JOIN genero g ON gf.genero_id = g.id;

-- 6. Em quais cinemas estão sendo exibidos filmes nacionais;

SELECT DISTINCT c.nomeFantasia, f.tituloPortugues FROM cinema c
INNER JOIN sala sa ON c.id = sa.cinema_id
INNER JOIN sessao s ON sa.id = s.sala_id
INNER JOIN filme f ON s.filme_id = f.id
WHERE f.pais_id = (SELECT id FROM pais p WHERE nome = 'Brasil');
